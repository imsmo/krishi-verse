// @krishi-verse/sdk-js · catalogue browse (GET /v1/catalogue/products) — platform-master + tenant products.
...
export class CatalogueResource {
  ...
  async browseProducts(query: { q?: string; categoryId?: string; cursor?: string; limit?: number } = {}, signal?: AbortSignal): Promise<Page<ProductCard>> {
    const r = await this.http.request<ProductCard[]>('GET', 'catalogue/products', { signal, query });
