// apps/mobile/src/app/(worker)/profile.tsx · screens 38/136/36 (worker profile + edit availability) + 139 (KYC
// status). Thin screen (guide §3): register a worker profile (if none) or edit prefs — travel radius, stay-away,
// min wage (₹→paise via BigInt, Law 2), emergency contact — via the pure buildWorkerPatch. Shows the 18+/KYC
// status (server-owned). Behind `worker_app`. Degrade-never-die. Skills (37/137) have no server field yet — flagged.
...
export default function WorkerProfileScreen() {
  ...
  if (!enabled) return <ScreenScaffold title={t('tabs.profile')}><EmptyState title={t('common.unavailable')} /></ScreenScaffold>;
    ...
    >
      {loading ? <SkeletonCard lines={4} /> : (
        <>
          ...
          <Text style={styles.note}>{t('worker.skillsNote')}</Text>
