// apps/mobile/src/app/(worker)/withdraw.tsx · screen 41 (withdraw earnings). Thin screen (guide §3): reuses the
// SAME features/wallet data layer + PURE `withdrawable` pre-check as the farmer wallet — a worker withdraws wages
// to a tokenised bank/UPI destination via a REAL, idempotent payout. The SERVER is the authority (balance/KYC/
// limits); the app never moves money (Law 11). FLAG_SECURE while shown (money screen). Behind `worker_active_job`
// AND `wallet`. Money is bigint paise (Law 2). Degrade-never-die. Re-uses the wallet i18n keys (no duplication).
...
export default function WorkerWithdraw() {
  ...
  const enabled = useFlag('worker_active_job') && useFlag('wallet');
