// core/http/zod.pipe.ts
// Validation pipes that run a zod schema over the request body / query and throw
// a structured 422 (ValidationError) on failure. Centralises validation so
// controllers stay declarative.
import { ArgumentMetadata, Injectable, PipeTransform } from '@nestjs/common';
import { ZodSchema } from 'zod';
import { Body, Query } from '@nestjs/common';
import { ValidationError } from '../../shared/errors/app-error';

function makePipe(schema: ZodSchema): PipeTransform {
  @Injectable()
  class ZodPipe implements PipeTransform {
    transform(value: unknown, _meta: ArgumentMetadata) {
      const r = schema.safeParse(value);
      if (!r.success) throw new ValidationError('Request validation failed', r.error.flatten() as any);
      return r.data;
    }
  }
  return new ZodPipe();
}
export const ZodBody = (schema: ZodSchema) => Body(makePipe(schema));
export const ZodQuery = (schema: ZodSchema) => Query(makePipe(schema));
