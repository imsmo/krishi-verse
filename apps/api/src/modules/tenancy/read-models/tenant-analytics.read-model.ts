...
// partitioned `orders` table (partition-pruned, Law 8); top-N lists are bounded. View-/impression-based "traffic"
// is intentionally OMITTED here (it needs the high-volume event pipeline — a separate wave), never faked.
...
export class TenantAnalyticsReadModel {
  ...
  async summary(tenantId: string, from: Date, to: Date, currency = 'INR'): Promise<TenantAnalytics> {
    ...
    const [orders, refunds, listings, disputes, payouts, topProducts, topSellers] = await Promise.all([
      db.query<any>(
