...
run('ai-governance spine (integration, real Postgres + RLS)', () => {
  ...
  beforeAll(async () => {
    ...
    inferenceSvc = new AiInferenceService(uow, new PgOutboxWriter(), new PgIdempotencyService(pools), new PromMetrics(), infRepo, reviewRepo, modelRepo);
    reviewSvc = new AiReviewService(uow, new PgOutboxWriter(), new PromMetrics(), audit, reviewRepo, infRepo);
    moderationSvc = new ModerationService(uow, new PgOutboxWriter(), new PromMetrics(), audit, reportRepo);
