...
(auctions + bids + EMD ledger bits), `offers_slice.sql` (listing_offers negotiation + 0020 RLS), `requirements_slice.sql` (reverse marketplace: requirements + requirement_responses), `logistics_slice.sql` (shipments + shipment_events fulfilment), `reviews_slice.sql` (verified-purchase reviews + eligibility), `disputes_slice.sql` (disputes + messages + eligibility), `promotions_slice.sql` (promotions + coupons + redemptions), `memberships_slice.sql` (tiers + user memberships), `tenancy_slice.sql` (plans + plan_limits + subscriptions + usage_counters), and `02_seed_min.sql`.
